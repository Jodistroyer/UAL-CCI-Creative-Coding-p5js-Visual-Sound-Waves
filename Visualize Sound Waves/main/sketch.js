...
let alphaValue = 100; // Initial alpha value
...
function draw() {
  ...
  bass = fft.getEnergy("bass") * 15000;
  mid = fft.getEnergy("mid") * 1500;
  treble = fft.getEnergy("treble") * 1500;
  ...
  alphaValue += 0.00000001; // You can adjust the increment value for a faster or slower effect
...
function drawDynamicWave(spectrum) {
  ...
  noFill();
  stroke(0, alphaValue); // Use dynamic alpha value
  strokeWeight(5);
  ...
  for (let i = 0; i < spectrum.length; i++) {
    ...
    let freqValue = map(spectrum[i], 10, 255, -height / 10, height / 4);
